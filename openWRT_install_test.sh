...
if [ "$iptables_inst" != "" ] 
	then
		echo 'remove iptable-Packages'
  		opkg remove iptable* --force-removal-of-dependent-packages
fi
...
uci commit mwan3 && reload_config
...
uci commit stubby && reload_config
...
uci commit unbound && reload_config 
...
mkdir -p /etc/dnsmasq.d/Blacklist
...
echo 'Restart at: '$(date +%d'.'%m'.'%y' '%H':'%M':'%S)
reboot 
