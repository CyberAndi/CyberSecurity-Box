...
return sid;},reorderSections:function(){var v=this.state.values,n=this.state.creates,d=this.state.deletes,r=this.state.reorder,tasks=[];if(Object.keys(r).length===0)
return Promise.resolve();for(var c in r){var o=[];if(d[c])
continue;if(n[c])
for(var s in n[c])
...
n[conf]={};n[conf][sid]={'.type':type,'.name':sid,'.create':name,'.anonymous':!name,'.index':1000+this.state.newidx++};return sid;},clone:function(conf,type,srcsid,put_next,name){let n=this.state.creates;let sid=this.createSID(conf);let v=this.state.values;put_next=put_next||false;if(!n[conf])
n[conf]={};n[conf][sid]={...v[conf][srcsid],'.type':type,'.name':sid,'.create':name,'.anonymous':!name,'.index':1000+this.state.newidx++};if(put_next)
this.move(conf,sid,srcsid,put_next);return sid;},remove:function(conf,sid){var v=this.state.values,n=this.state.creates,c=this.state.changes,d=this.state.deletes;if(n[conf]&&n[conf][sid]){delete n[conf][sid];}
...
if(v[conf]){if(d[conf]&&d[conf][sid]===true)
return null;var s=v[conf][sid]||null;if(s){if(c[conf]&&c[conf][sid])
for(var opt in c[conf][sid])
if(c[conf][sid][opt]!=null)
s[opt]=c[conf][sid][opt];if(d[conf]&&d[conf][sid])
for(var opt in d[conf][sid])
delete s[opt];}
return s;}
return null;},set:function(conf,sid,opt,val){var v=this.state.values,n=this.state.creates,c=this.state.changes,d=this.state.deletes;sid=this.resolveSID(conf,sid);if(sid==null||opt==null||opt.charAt(0)=='.')
...
c[conf][sid]={};if(d[conf]&&d[conf][sid]){if(isEmpty(d[conf][sid],opt))
delete d[conf][sid];else
delete d[conf][sid][opt];}
c[conf][sid][opt]=val;}
else{if(c[conf]&&c[conf][sid]){if(isEmpty(c[conf][sid],opt))
delete c[conf][sid];else
delete c[conf][sid][opt];}
if(v[conf]&&v[conf][sid]&&v[conf][sid].hasOwnProperty(opt)){if(!d[conf])
